...
import '../../core/ui/exceptions/user_not_found_exception.dart';

class AuthRepositoryImpl implements AuthRepository {
  final RestClient _restClient;

  AuthRepositoryImpl({required RestClient restClient}) : _restClient = restClient;

  @override
  Future<UserModel> register(String name, String email, String password) async {
    final result = await _restClient.post('/auth/register', {'name': name, 'email': email, 'password': password});
    if (result.hasError) {
  ...
  Future<UserModel> login(String email, String password) async {
    ...
    if (result.hasError) {
      ...
      log('Erro ao autenticar usuário', error: result.statusText, stackTrace: StackTrace.current);
